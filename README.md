...
```powershell
.\scripts\init-env.ps1 -SourceDirs "D:\path\to\audio-folder" -OutputDirs "D:\path\to\transcripts"
```

Generate `.env` on Linux or macOS:

```sh
sh ./scripts/init-env.sh --source-dir /path/to/audio-folder --output-dir /path/to/transcripts
```
...
To configure multiple paired folders, pass paths in the same order:

```powershell
.\scripts\init-env.ps1 -SourceDirs "D:\audio-a" "D:\audio-b" -OutputDirs "E:\out-a" "E:\out-b"
```

```sh
sh ./scripts/init-env.sh --source-dir /audio-a --output-dir /out-a --source-dir /audio-b --output-dir /out-b
```

The source folders should normally live outside this repo. Transcripts are written only to the paired output folders.
...
OUTPUT_DIRS=D:/path/to/transcripts
...
```powershell
.\scripts\init-env.ps1 -SourceDirs "D:\path\to\audio-folder" -OutputDirs "D:\path\to\transcripts" -Cuda
```

```sh
sh ./scripts/init-env.sh --source-dir /path/to/audio-folder --output-dir /path/to/transcripts --cuda
```
...
```text
/path/to/transcripts/audio-folder_20260517161230/song_created-20260517143000_modified-20260517154512.txt
/path/to/transcripts/audio-folder_20260517161230/song_created-20260517143000_modified-20260517154512.json
/path/to/transcripts/audio-folder_20260517161230/song_created-20260517143000_modified-20260517154512.tsv
/path/to/transcripts/audio-folder_20260517161230/song_created-20260517143000_modified-20260517154512.srt
/path/to/transcripts/audio-folder_20260517161230/song_created-20260517143000_modified-20260517154512.vtt
```

For each source/output pair, transcript files are written into:

```text
<paired_output_folder>/<source_folder_name>_<YYYYMMDDHHMMSS>/
```

Relative subfolders are preserved in the paired output folder. Transcript sidecars are not written beside the original media files.
...
- `OUTPUT_DIRS`: semicolon-separated host transcript output folders; generated by setup scripts
